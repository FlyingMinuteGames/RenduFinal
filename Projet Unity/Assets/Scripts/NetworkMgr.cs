...
public class NetworkMgr : MonoBehaviour {
	...
	void Update () {
        ...
        else
        {
            ...
            {
                PlayAnnouncement(2);
                timer_worldchange = 30 + Random.Range(0, 10);
    ...
    private Callback[] action_callback = {
                                            ...
                                            (me,enable) => { if(enable && me.timer_bomb <= 0){ me.networkView.RPC("ServerRecvDropBomb",RPCMode.Server, Network.player); me.timer_bomb = 4;} return 0;}
                                          };
    ...
    public void HandleRespawn(int index)
    {
        if (index < 0 || index > 3)
            return;

        controllers[index].transform.position = GetInitPos(index);
        networkView.RPC("ClientRecvRespawn", RPCMode.Others, index, controllers[index].transform.position);
    }
    ...
    [RPC]
    void ClientRecvRespawn(int pIndex,Vector3 pos)
    {

        if (pIndex >= 0)
        {
            controllers[pIndex].transform.position = pos;
        }
